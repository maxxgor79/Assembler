...
public class PreferencesDialog extends JDialog implements ModalDialog {
    ...
    private static final int LINE_ENDING = 12;
    ...
    protected void store() {
        ...
        store.putObject(LINE_ENDING, preferencesTabbedPane.getMiscellaneousPanel().getLineEndingPanel().getCbLineEnding()
                .getSelectedItem());
    }
    ...
    protected void restore() {
        ...
        if (store.contains(LINE_ENDING)) {
            try {
                preferencesTabbedPane.getMiscellaneousPanel().getLineEndingPanel().getCbLineEnding()
                        .setSelectedItem(store.getObject(LINE_ENDING));
            } catch (StoreException e) {
                log.error(e.getMessage(), e);
            }
        }
