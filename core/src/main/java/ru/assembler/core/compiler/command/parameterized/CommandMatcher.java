...
public class CommandMatcher {
  ...
  private static int skipExpression(List<Lexem> commandList, int commandIndex) {
    final IndexedArrayIterator indexedArrayIterator = new IndexedArrayIterator<>(commandList,
        commandIndex);
    final ExpressionConsumer expressionConsumer = new ExpressionConsumer(indexedArrayIterator);
    final Lexem lastLexem = expressionConsumer.evaluate();
    if (lastLexem == null) {
