...
import static ru.assembler.core.util.TypeUtil.isAddressOffsetPattern;
import static ru.assembler.core.util.TypeUtil.isIntegerPattern;
import static ru.assembler.core.util.TypeUtil.isOffsetPattern;
...
public class ParameterizedCommandCompiler implements CommandCompiler {
  ...
  protected BigInteger compileExpressionNumber(LexemSequence command, Lexem patternLexem,
    ...
    final Expression expression = new Expression(compilerApi.getFile(), commandIterator,
        namespaceApi);
    ...
    while (patternIterator.hasNext()) {
      ...
      if (patternLexem.getType() == LexemType.VARIABLE) {
        ...
        if (!patternLexem.equals(commandLexem)) {
          throw new CompilerException(compilerApi.getFile(), commandLexem.getLineNumber(),
              MessageList
